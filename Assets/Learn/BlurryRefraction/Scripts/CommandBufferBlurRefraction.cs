...
public class CommandBufferBlurRefraction : MonoBehaviour
{
	...
	public void OnWillRenderObject()
	{
		...
		buf.GetTemporaryRT (screenCopyID, -1, -1, 0, FilterMode.Point); //生成名字为_ScreenCopyTexture的RenderTexture并给编号id赋值
		//当前屏幕的纹理赋值给screenCopyID标识的纹理
		buf.Blit (BuiltinRenderTextureType.CurrentActive, screenCopyID);

		//获取2个模板纹理, 而后将它们2的效果轮流混合
		int blurredID = Shader.PropertyToID("_Temp1");
		int blurredID2 = Shader.PropertyToID("_Temp2");
		buf.GetTemporaryRT (blurredID, -2, -2, 0, FilterMode.Point);
		buf.GetTemporaryRT (blurredID2, -2, -2, 0, FilterMode.Point);

		//将抓屏纹理填充到_Temp1后释放
		buf.Blit (screenCopyID, blurredID);
		buf.ReleaseTemporaryRT (screenCopyID);  //释放编号id的rendertexture 
